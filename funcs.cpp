...
int findshift(std::array<double,26> arr) {
  int index1,index2 = 0;
  double mostfrequent = arr.at(0);
  for (int i = 0;i < 26;i++) {
    if (arr.at(i) > mostfrequent) {
      mostfrequent = arr.at(i);
      index1 = i;
    }
  }
  ...
  double mostfrequent2 = freqOfLangChar[0];
  for (int i = 0;i < 26;i++) {
    if (freqOfLangChar[i] > mostfrequent2) {
      mostfrequent2 = freqOfLangChar[i];
      index2 = i;
    }
  }
  /*for (int i = 0; i < 26;i++) {
  ...
  int estimatedShift = index2 - index1;
  return estimatedShift;
}
...
std::string solve(std::string encrypted_string) {
  ...
  int shift = -1*findshift(frequencyChart);
